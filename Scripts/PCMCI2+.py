...
def get_domain_knowledge_links(var_names, tau_min, tau_max, flu_label):
    ...
    V2: Fix KeyError caused by duplicate parent entries.
    Strategy: Sunshine is Exogenous (only auto-regressive).
    """
    selected_links = {}
    ...
    ss_idx = var_dict.get('SS')
    if ss_idx is None: 
        ss_idx = var_dict.get('Sunshine')
    ...
    for idx, var in enumerate(var_names):
        ...
        if var == flu_label:
            parents = [
                (other_idx, -tau) 
                for other_idx in range(len(var_names))
                for tau in range(tau_min, tau_max + 1)
            ]
        
        # CASE 2: Sunshine (Pure Exogenous)
        # Sunshine 不受 T/AH 影响，只受自身历史影响 (物理约束：天气系统驱动地面气象)
        elif var in ss_vars:
            if ss_idx is not None:
                parents = [(ss_idx, -tau) for tau in range(tau_min, tau_max + 1)]
            
        # CASE 3: Temperature & Humidity (Semi-Exogenous)
        # T 和 AH 互为因果，受 Sunshine 影响，受自身影响
        elif var in met_vars:
            # 1. 自身滞后 (由下面的循环统一处理，避免重复)
            # 如果当前是 T，则 temp_idx == idx
            # 如果当前是 AH，则 humid_idx == idx
            
            # 添加 T 的影响
            if temp_idx is not None:
                parents += [(temp_idx, -tau) for tau in range(tau_min, tau_max + 1)]
            
            # 添加 AH 的影响
            if humid_idx is not None:
                parents += [(humid_idx, -tau) for tau in range(tau_min, tau_max + 1)]
            
            # 添加 Sunshine 的影响
            if ss_idx is not None:
                parents += [(ss_idx, -tau) for tau in range(tau_min, tau_max + 1)]
                
        # CASE 4: Pollutants (Intermediate)
        # 受自身、气象、日照影响
        else:
            # 自身
...
if __name__ == '__main__':
    ...
    OUT_DIR = os.path.join(BASE_DIR, 'Figures/PCMCI2+')
    ...
    for city in cities:
        ...
        for flu in flus:
            ...
            for pol in pollutants:
                ...
                required_cols = ['date', flu, 'ave.temp', 'humidity', 'sunshine', pol_col]
                ...
                internal_vars = ['T', 'AH', pol, 'SS', 'Flu']
                
                # LaTeX Label for Plotting
                flu_tex = r'$Flu_A$' if flu == 'FLUA' else r'$Flu_B$'
                display_names = ['T', 'AH', LATEX_LABELS.get(pol, pol), LATEX_LABELS.get('SS', 'SS'), flu_tex]
